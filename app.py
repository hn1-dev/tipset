...
# ── HEADER ───────────────────────────────────────────────────────────────────
st.markdown(f"<h1 style='text-align:center;letter-spacing:.1em;margin-bottom:.2rem;color:{YELLOW}'>MAZARINTIPSET</h2>", unsafe_allow_html=True)
...
st.markdown(f"<h3 style='color:{YELLOW}'>Points per Team per Player — Week {MAX_WEEK}</h3>", unsafe_allow_html=True)
